...
# Whisper sometimes emits full English language names instead of ISO codes.
# Normalize them before validation so persisted long-form values don't cause silent drops.
LANGUAGE_LONG_TO_ISO: dict[str, str] = {
...
class TranscriptionSegment(BaseModel):
    ...
    def validate_language(cls, v):
        ...
        if v not in ACCEPTED_LANGUAGE_CODES:
            normalized = LANGUAGE_LONG_TO_ISO.get(v.lower())
            if normalized:
                return normalized
            raise ValueError(f"Invalid language code '{v}'. Must be one of: {sorted(ACCEPTED_LANGUAGE_CODES)}")
        return v
